...
CHIP_ERROR NFCBase::Init(const NfcListenParameters & params)
{
    ...
    chip::DeviceLayer::Internal::NFCMgrImpl().SetNFCBase(this);
...
CHIP_ERROR NFCBase::SendMessage(const Transport::PeerAddress & address, System::PacketBufferHandle && msgBuf)
{
    ...
    ReturnErrorCodeIf(address.GetTransportType() != Type::kNfc, CHIP_ERROR_INVALID_ARGUMENT);
    ReturnErrorCodeIf(mState == State::kNotReady, CHIP_ERROR_INCORRECT_STATE);

    chip::DeviceLayer::Internal::NFCMgrImpl().SendToNfcTag(std::move(msgBuf));
